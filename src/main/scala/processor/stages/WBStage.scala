...
class WBStage extends Module {
  ...
  val alu_result                   = RegNext(io.alu_result_MEMtoWB)
  val write_back_select            = RegNext(io.write_back_select_MEMtoWB)
  val address_is_io                = RegNext(io.address_is_io_MEMtoWB)
  io.register_write_enable_WBtoEX := RegNext(io.register_write_enable_MEMtoWB)
  io.rd_WBtoEX                    := RegNext(io.rd_MEMtoWB)

  // Implements multiplexers in diagram
  when(write_back_select){
    io.regfile_write_data_WBtoEX := Mux(address_is_io,io.io_memory_read_MEMtoWB,io.data_memory_read_MEMtoWB)
  } .otherwise {
    io.regfile_write_data_WBtoEX := alu_result
  }
}
